...
`v2.2.0` 不新增业务功能，版本主题聚焦 **工程质量、可扩展性与规范化治理**。本次优化优先处理跨模块会持续放大的技术债：异常出口、参数校验、前端 lint 入口、版本基线、CodePen 控制器重复样板、前端静态错误与生产 Mapper SQL 规范。
...
- **版本基线统一**：后端 Maven、管理端前端、用户端前端、Jar 命令与 Docker 镜像标签统一升级到 `v2.2.0`。
...
# 或直接运行打包后的 jar
java -jar xiaou-application/target/xiaou-application-v2.2.0.jar --spring.profiles.active=prod
```
...
```bash
# 构建镜像
docker build -t code-nest:v2.2.0 -f docker/Dockerfile .
...
### v2.2.0 🛠️ Code Quality Hardening
...
- 📦 **版本基线升级**：后端 Maven、管理端前端、用户端前端、README Jar/Docker 示例统一升级到 `v2.2.0`。

...
- **AI治理与质量兜底**：`xiaou-ai` 统一登记 Coze 工作流，管理端提供 AI 治理总览，可查看配置状态、兜底覆盖率与风险建议。
...
| xiaou-ai | 统一 AI 服务 | Coze 工作流调用、AI 结果兜底、治理总览 |
...
- 🤖 **AI Runtime 轻量治理**：新增 AI 治理总览接口与管理端页面，展示 Coze 工作流目录、配置状态、兜底覆盖率、风险等级与治理建议。

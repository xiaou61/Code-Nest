...
- **版本历史重建**：线上版本历史按 Git 提交与版本分支重新整理为 `v1.0.0` 至 `v2.3.1` 的 27 条发布记录，并修复时间轴分页总数不准确的问题。
...
# 或直接运行打包后的 jar
java -jar xiaou-application/target/xiaou-application-v2.3.1.jar --spring.profiles.active=prod
```
...
```bash
# 构建镜像
docker build -t code-nest:v2.3.1 -f docker/Dockerfile .

...
- 管理端智能体工作台新增“对话 / 持久任务”显式模式，支持创建、筛选、选择和刷新当前管理员拥有的后台任务。
- 新增任务生命周期、进度、持久步骤、trace/audit 元数据和游标事件时间线；移动端使用完整的步骤摘要布局。
- 新增 `WAITING_INPUT`、`PAUSED` 和任务事件控制面，任务所有者可以安全暂停、恢复、补充结构化输入或取消任务。

### Changed

- 每次任务领取使用唯一租约令牌，长时间规划或工具执行期间持续续约，并以扫描截止时间原子隔离陈旧任务恢复。
- 单任务恢复失败不再回滚同批其他任务；租约续约与恢复指标仅使用固定低基数结果标签。
- 前端只在工作台激活且任务未终止时轮询，事件按稳定 cursor 去重并保持有界；原有 `/admin/agent/chat` 行为与会话状态保持不变。

### Security

- 暂停、恢复、取消、补充输入和确认控件严格跟随后端状态机；补充输入仅接受有界非空 JSON 对象。
- 写风险步骤继续要求后端预览、审计和完全匹配的强确认文本，浏览器不推断或直接执行破坏性动作。

### Migration

- 数据库从远端 `v2.5.8` 升级时，依次应用 `sql/v2.5.9/admin_agent_tasks.sql` 和 `sql/v2.5.10/admin_agent_workflow_control_plane.sql`，schema 基线更新为 `v2.5.10`。
- 任务 Worker 仍默认关闭；完成两段迁移、表结构和指标验证后，再通过 `XIAOU_ADMIN_AGENT_TASK_ENABLED=true` 单实例灰度开启。

### Verification

- OpenSpec 主规格严格校验、任务状态/控制面/租约/接口测试、管理端 44 项契约测试、双前端与文档站生产构建、发布清单一致性及仓库 architecture/hygiene 门禁。

## [v2.5.9] - 2026-08-05

### Added

- 管理端智能体从固定宽度抽屉升级为全屏三栏工作台，统一承载会话、执行详情和结构化结果。
- 新增有界多会话持久化、会话搜索、新建/删除、消息复制/重试与 Markdown 导出。
- 新增计划、差异、产物、下一步建议、审计元数据和运行 trace 检查器；表格型产物优先按列渲染。
- 统一聊天请求支持前端取消等待，写入动作仍继续经过后端预览、强确认和审计链路。
- 新增持久化管理员智能体任务 API、MySQL 任务/步骤状态、LangGraph4j 单周期执行、默认关闭的有界 Worker 和完整任务指标。

### Changed

- 智能体回复使用安全 Markdown 渲染，交互控件补齐 ARIA 标签、动态内容播报、键盘焦点和 reduced-motion 降级。
- 桌面端使用稳定三栏布局，窄屏与移动端将会话列表和检查器切换为可关闭侧边面板。
- 多步骤任务复用原有注册工具、实时角色/权限、policy、预览、强确认和审计链路；只读中断可恢复，结果不明的已确认写步骤进入人工复核且不自动重放。

### Migration

- 新增 `sql/v2.5.9/admin_agent_tasks.sql`，数据库 schema 基线更新为 `v2.5.9`。
- 新增 `XIAOU_ADMIN_AGENT_TASK_*` 运行参数；Worker 默认关闭，必须在迁移和监控验证后灰度开启。
- 历史会话保存于当前管理员浏览器，最多 20 个会话、每个会话 100 条消息；尚不提供跨设备服务端同步。

### Verification

- 会话状态与导出单元测试、统一聊天前端契约、任务状态机/恢复/Worker/指标/API 测试、OpenSpec 严格校验、管理端完整契约套件及 Vite 生产构建。

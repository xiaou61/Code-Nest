...
1. `npm ci` 和 `npm run build` 在干净环境中通过。
2. 如果部署到子路径，`VITEPRESS_BASE` 与访问路径一致。
3. 静态服务器启用了 `try_files` 或等价的 SPA 回退规则。
4. `code-nest-mark.svg` 等静态资源在发布环境能访问。
5. 发布后打开首页、模块页、搜索和至少一个深层路由。

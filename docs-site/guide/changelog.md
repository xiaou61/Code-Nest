...
| 版本 | 日期 | 主要变更 |
|------|------|---------|
| v2.5.10 | 2026-08-10 | 持久任务产品工作台、事件控制面、人工干预与租约可靠性加固 |
| v2.5.9 | 2026-08-05 | 管理端全屏智能体工作台、持久化多步骤任务、安全确认/恢复、任务运行指标 |
| v2.5.8 | 2026-08-05 | OpenAI-compatible 推理等级透传与 AI Provider 专用代理 |
| v2.5.7 | 2026-08-05 | SRE Worker 生产启动修复与发布 sudo 契约校验 |
...
- 管理端智能体新增持久任务模式，支持任务创建、筛选、详情、步骤、终态原因和响应式移动布局。
- 新增稳定 cursor 的任务事件时间线，以及 `WAITING_INPUT`、`PAUSED`、恢复、取消和补充输入控制。
- 新增唯一 lease token、长周期 heartbeat 续约和低基数 lease/recovery 指标。

### Changed

- 陈旧任务恢复使用扫描 heartbeat cutoff 原子 fencing，并按任务独立事务处理冲突或异常。
- 任务工作台仅轮询活跃非终态任务；写风险操作继续经过权限、预览、审计和精确强确认。
- 原有 `/admin/agent/chat` 单工具契约和浏览器端会话状态保持兼容。

### Migration

- 从 `v2.5.8` 升级时依次应用 `sql/v2.5.9/admin_agent_tasks.sql` 和 `sql/v2.5.10/admin_agent_workflow_control_plane.sql`，schema 基线更新为 `v2.5.10`。
- Worker 默认关闭，完成表结构、事件写入和指标验证后再单实例灰度开启。
...
- 新增全屏管理员智能体工作台，会话、执行详情和结构化产物使用三栏布局。
- 新增会话搜索、新建/删除、重试、复制、导出和浏览器端有界持久化。
- 新增表格型产物渲染和 plan/diff/trace/audit 检查器。
- 新增 `/admin/agent/tasks` 持久化多步骤任务、MySQL 状态机、默认关闭的 Worker、写确认恢复和低基数运行指标。

### Migration

- 新增 `sql/v2.5.9/admin_agent_tasks.sql`；Worker 默认关闭，迁移与指标验证后通过 `XIAOU_ADMIN_AGENT_TASK_ENABLED=true` 灰度开启。
- 会话历史当前仅保存于本机浏览器，不进行跨设备同步。

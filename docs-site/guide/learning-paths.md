...
| 9 | [发布前验证](/guide/release-verification) | 改不同范围时应该跑哪些构建、接口和页面烟测 |
...
| 4 | [问题定位流程](/operations/diagnosis-flow) | 遇到未知问题时如何从现象定位到模块和依赖 |
| 5 | [常见问题排查](/operations/troubleshooting) | 后端启动、401、CORS、WebSocket、文件 URL 等问题 |
| 6 | [发布前验证](/guide/release-verification) | 发布前最终清单和 CI 覆盖边界 |
| 7 | [版本历史](/modules/version-history) | 版本记录、发布状态和用户可见更新说明 |
...
| 4 | [全功能覆盖矩阵](/reference/feature-coverage) | 检查功能是否有用户入口、管理入口、API、表和模块页 |
| 5 | [模块总览](/modules/) | 模块页统一模板和依赖关系 |
| 6 | [核心链路教程](/manuals/core-workflows) | 用户视角的端到端流程 |
| 7 | [验证记录与已知问题](/manuals/verified-scenarios) | 真实验证结果和已知缺口 |
| 8 | [v2.2.0 文档计划](/roadmap/v2.2.0-docs-plan) | 已完成阶段和后续补强节奏 |

...
需要直接分配首单题目时，可以继续用 [首单任务题库](/guide/first-task-backlog)。

如果首单已经跑完，想判断什么时候可以继续接模块、什么时候还该先保护着做，继续看 [模块接手升级路线](/guide/module-ownership-ladder)。

如果已经准备真正接一个模块，而不是只做单点任务，继续用 [模块接手检查清单](/guide/module-takeover-checklist) 会更落地。

如果你更想直接看“合格接手记录大概长什么样”，继续接 [模块接手样例包](/guide/module-takeover-examples)。

如果你已经在持续接某个模块，准备从“会接”进到“会守”，继续看 [模块值守与回归手册](/guide/module-owner-playbook)。
...
| 10 | [模块状态机与生命周期索引](/reference/module-state-machines) | 先建立 OJ、学习资产、模拟面试、版本发布、小组申请等高风险状态脑图 |
| 11 | [测试与回归](/guide/testing-regression) | 按改动范围选择构建、测试、烟测和记录方式 |
| 12 | [端到端业务链路图](/manuals/business-flow-map) | 从注册、学习、AI、OJ、社区、聊天到后台运营看全局闭环 |
| 13 | [发布前验证](/guide/release-verification) | 改不同范围时应该跑哪些构建、接口和页面烟测 |
...
| 5 | [事故响应](/operations/incident-response) | 线上事故先怎么止损、怎么分级、怎么留证据 |
| 6 | [问题定位流程](/operations/diagnosis-flow) | 遇到未知问题时如何从现象定位到模块和依赖 |
| 7 | [常见问题排查](/operations/troubleshooting) | 后端启动、401、CORS、WebSocket、文件 URL 等问题 |
| 8 | [发布前验证](/guide/release-verification) | 发布前最终清单和 CI 覆盖边界 |
| 9 | [版本历史](/modules/version-history) | 版本记录、发布状态和用户可见更新说明 |
...
适合要继续补当前文档工作线、维护模块页、写操作手册或同步新功能的人。
...
如果你已经开始连续做真实任务，下一步最值得补的不是“再多看几篇文档”，而是回到 [模块接手升级路线](/guide/module-ownership-ladder) 判断自己现在属于 S1、S2、S3 还是更高阶段。

当你确认自己已经接近 `S3/S4` 时，继续配合 [模块接手检查清单](/guide/module-takeover-checklist) 去盘真实模块，会比继续泛读文档更有效。

盘完以后，最好再拿 [模块接手样例包](/guide/module-takeover-examples) 做一次对照，看看自己是不是漏了状态分层、失败态或文档同步点。

当你已经开始持续接手模块任务后，再把 [模块值守与回归手册](/guide/module-owner-playbook) 接上，会更像一条完整成长链。
...
| 文档 | 说明 |
| --- | --- |
| [按模块学习路线](/guide/module-learning-paths) | 模块学习路线 |
| [模块学习任务模板](/guide/module-learning-workbook) | 学习任务模板 |

...
如果你是新人准备接第一单，先读 [首个真实任务接入指南](/guide/first-real-task)，再回到本页走完整开发流程会更稳。

如果你还没有现成题目，可以先从 [首单任务题库](/guide/first-task-backlog) 选一条合适的任务。

如果只修一个小文案或样式，可以不完整走完这页。只要变更涉及接口、表、权限、AI、文件、通知、积分、WebSocket 或部署配置，就建议按这页过一遍。

如果你已经知道自己属于“新增用户端页面、后台管理页、AI 场景、内容类型、积分规则”等常见场景，可以直接参考 [常见二开场景](/guide/extension-scenarios)。

如果这次变更涉及权限、文件、内容渲染、敏感词、WebSocket、AI 或部署配置，先过一遍 [权限与安全边界](/guide/security-boundaries)。改完以后按 [测试与回归](/guide/testing-regression) 选择验证层级。
...
如果状态会跨多个步骤推进，要优先在模块页写成状态机，而不是只写“保存成功”。OJ 提交、学习资产审核、模拟面试会话、版本发布、文件迁移都属于这类功能。还没建立整体脑图时，可以先对照 [模块状态机与生命周期索引](/reference/module-state-machines)。
...
如果这个功能在成功后还会触发通知、积分、排行、日志、ACK 或其他回流，建议先看 [事件、通知与回流索引](/reference/event-backflow-index)，把“做完以后会回到哪里”先定下来。
...
如果走 GitHub PR 流程，仓库根目录的 `.github/pull_request_template.md` 已经把“验证结果”和“文档同步清单”固定下来了，建议直接按模板填，避免只写代码改动，不写依赖状态和未验证边界。

## 实战演练：以版本历史模块为例

下面用一个完整的端到端例子，演示如何从 0 把一个功能写到可合并。我们选 `xiaou-version`（版本历史）作为参考模块，因为它结构完整且不过于复杂。
...
| 项 | 判断 |
| --- | --- |
| 新模块还是已有增强 | 新模块 `xiaou-version` |
| 用户端入口 | `/version-history` 页面，浏览已发布版本列表 |
| 管理端入口 | `/system/version`，CRUD + 发布/隐藏 + 批量操作 |
| 后端 Maven 模块 | `xiaou-version`，依赖 `xiaou-common` |
| 新表 | `version_history`（1 张主表，无明细表） |

### 第二步：设计数据和状态

`version_history` 表核心字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id | BIGINT AUTO_INCREMENT | 主键 |
| version_number | VARCHAR(32) | 版本号，唯一约束 |
| title | VARCHAR(128) | 更新标题 |
| update_type | TINYINT | 1=重大更新 2=功能更新 3=修复更新 4=其他 |
| description | TEXT | 简要描述 |
| status | TINYINT | 0=草稿 1=已发布 2=已隐藏 |
...
唯一约束：`version_number`（同一版本号不能重复出现）。

### 第三步：写后端

1. **pom.xml** — 只依赖 `xiaou-common`：

```xml
<artifactId>xiaou-version</artifactId>
<dependencies>
    <dependency>
...
public class VersionHistory {
    ...
    private Integer updateType;  // 1-4
    private String description;
    private Integer status;      // 0=草稿 1=已发布 2=已隐藏
...
3. **admin Controller** — 每个写方法加 `@RequireAdmin` 和 `@Log`：
...
public class VersionHistoryAdminController {
    ...
    @RequireAdmin
    @PostMapping("/list")
    public Result<PageResult<VersionHistoryResponse>> getVersionList(
        @RequestBody VersionHistoryQueryRequest request) {
        return Result.success(versionHistoryService.getAdminVersionList(request));
    }

    @RequireAdmin
    @Log(module = "版本管理", type = Log.OperationType.INSERT,
         description = "创建版本历史记录")
    @PostMapping("/create")
    public Result<Long> createVersion(
        @Validated @RequestBody VersionHistoryCreateRequest request) {
        return Result.success(versionHistoryService.createVersion(request));
    }

    @RequireAdmin
    @Log(module = "版本管理", type = Log.OperationType.UPDATE,
         description = "发布版本")
    @PostMapping("/publish")
    ...
    // ... update, delete, hide, unpublish, batch 操作同理
}
```

4. **Service** — 唯一性校验 + 管理员身份取法：
...
public class VersionHistoryServiceImpl implements VersionHistoryService {
    ...
    public Long createVersion(VersionHistoryCreateRequest request) {
        ...
        BeanUtil.copyProperties(request, vh);
        vh.setCreatedBy(StpAdminUtil.getLoginIdAsLong());
        vh.setViewCount(0);
...
| 模式 | 在版本历史中的体现 |
| --- | --- |
| 管理员身份 | `StpAdminUtil.getLoginIdAsLong()` |
| 业务异常 | `throw new BusinessException("版本号已存在")` |
| 写操作日志 | `@Log(module = "版本管理", ...)` |
| 参数校验 | `@Validated @RequestBody` |
| 逻辑删除 | `logicalDeleteById(id, adminId)` |
...
- 路由：`/version-history`
- 组件：`views/version-history/Index.vue`
- API：`api/version.js` → `getPublishedList()`

管理端：

- 路由：`/system/version`
- 组件：`views/system/version/index.vue`
- API：`api/version.js` → `getAdminList()`, `createVersion()`, `publishVersion()`

两端都需要 Vite proxy 把 `/api` 转发到 `http://localhost:9999`。

### 第五步：接入横切能力

| 能力 | 是否需要 | 怎么接 |
| --- | --- | --- |
| 鉴权 | 需要 | 管理端写操作加 `@RequireAdmin`，用户端读取走白名单 |
| 通知 | 不需要 | 版本发布不触发通知（当前需求） |
| 积分 | 不需要 | 浏览版本不涉及积分 |
| 敏感词 | 可选 | 如果 description 允许用户输入，需接入 |
| 文件 | 不需要 | 版本记录无附件 |
| 操作日志 | 需要 | 所有管理端写操作加 `@Log` |

### 第六步：同步文档

| 变更 | 更新的文档 |
| --- | --- |
| 新模块 | `modules/version-history.md`（新建） |
| 新 API | `reference/api-routes.md`（加一行 `/admin/version` 和 `/version`） |
| 新路由 | `reference/frontend-routes.md`（加用户端和管理端路由） |
| 新表 | `reference/database-tables.md`（加 `version_history` 行） |
| 覆盖矩阵 | `reference/feature-coverage.md`（加"版本历史"行） |
...
5. 文档构建：`cd docs-site && npm run build`

...
| --- | --- | --- | --- | --- |
| 社区帖子 | `/community`、帖子详情 | `/community/posts` | `xiaou-community` | [社区帖子](/modules/community) |
...
| 博客 | `/blog`、博客主页、文章详情 | `/blog/articles` | `xiaou-blog` | [博客](/modules/blog) |
| 代码工坊 | `/codepen`、编辑器、作品详情 | `/codepen/pens` | `xiaou-codepen` | [代码工坊](/modules/codepen) |
...
| 内容 | 适合承载 | 特点 |
| --- | --- | --- |
| 社区帖子 | 技术讨论、问题求助、经验复盘 | 有分类、标签、评论、收藏、AI 摘要 |
| 动态 | 轻量状态、碎片记录、短内容互动 | 发布频率限制更明显，热门榜按互动刷新 |
| 博客 | 长文、体系化总结、系列文章 | 有个人博客主页、开通成本、发布成本 |
| CodePen | 前端作品、代码片段、模板 | 可在线编辑、发布、Fork、付费 Fork |
| 学习资产 | 可复习材料、练习计划、知识节点候选 | 来自内容转化，强调审核和二次学习 |
...
| 模块 | 关键状态 |
| --- | --- |
| 社区帖子 | 关注帖子状态、分类/标签启用状态、社区用户禁言状态 |
| 动态 | `0` 删除、`1` 正常、`2` 审核中 |
| 博客文章 | `0` 草稿、`1` 已发布、`2` 下线、`3` 删除 |
| CodePen 作品 | `0` 草稿、`1` 已发布、`2` 下线、`3` 删除 |
| 学习资产记录 | 记录状态和候选状态区分直接发布与审核发布 |
| 敏感词策略 | `replace`、`reject`、`warn` 等动作 |
...
| 模块 | 热度口径 |
| --- | --- |
| 社区帖子 | `like * 3 + comment * 5 + collect * 8 + view * 0.1` |
| 动态广场 | `like * 2 + comment * 3 + view * 0.1` |
| CodePen | 关注浏览、点赞、收藏、Fork、收益 |
| 博客 | 关注文章浏览、分类标签、开通/发布积分成本 |
...
| 社区帖子 | 知识节点候选、闪卡、练习计划 |
| 博客文章 | 闪卡、知识节点候选、练习计划 |
| CodePen 作品 | 练习计划、作品复盘材料 |
| 模拟面试报告 | 闪卡、薄弱项练习、知识节点候选 |
...
| 风险 | 处理 |
| --- | --- |
| 敏感词 | 通过敏感词模块做预处理、AC 匹配、白名单和策略动作 |
| 用户禁言 | 社区用户状态或聊天室禁言会阻止对应发布/发送 |
| 高频发布 | 动态模块限制 5 分钟 3 条 |
| 分类/标签下线 | 社区发帖会校验分类和标签是否启用 |
| 评论层级膨胀 | 社区评论固定为两级树 |
| AI 摘要失败 | 社区摘要有开关和缓存，失败不能阻断帖子展示 |
| 付费 Fork | CodePen 需要积分扣减、作者收益和作品复制在事务内一致 |
...
| 域 | 核心表 |
| --- | --- |
| 社区 | `community_post`、`community_comment`、`community_category`、`community_tag`、`community_user_status` |
| 动态 | `moment`、`moment_comment`、`moment_like`、`moment_collect` |
| 博客 | `blog_article`、`blog_config`、`blog_category`、`blog_tag` |
| CodePen | `code_pen`、`code_pen_template`、`code_pen_fork_transaction`、作品互动相关表 |
| 学习资产 | 学习资产记录表、候选表、发布目标相关表 |
| 敏感词 | `sensitive_word`、`sensitive_check_log`、`sensitive_strategy`、`sensitive_whitelist` |

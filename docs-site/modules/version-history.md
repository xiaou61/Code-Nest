...
| 来源 | 用途 | 当前示例 |
| --- | --- | --- |
| Git 分支 | 正在开发哪条工作线 | `v2.4.3` |
| Maven `revision` 与前端 `package.json` | 当前构建产物声明的版本 | `v2.4.3` |
| Git tag | 已形成的正式发布锚点 | `v2.4.1`，待本次发布创建 `v2.4.3` |
...
因此不要仅凭分支名宣布发布，也不要因为 package 版本尚未提升就忽略正在进行的开发线。当前统一口径是：**开发线和构建产物为 `v2.4.3`；最近正式 tag 为 `v2.4.1`，验证完成后再创建 `v2.4.3` tag。**
...
- **当前发布线**：`v2.4.3`
- **当前构建产物版本**：`v2.4.3`，以 Maven `revision` 和各前端 `package.json` 为准
- **最近正式 tag**：`v2.4.1`，验证完成后创建 `v2.4.3`

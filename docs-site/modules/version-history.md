...
2. 再看 `v2.0.0` 到 `v2.5.0` 的关键节点，理解 AI Runtime、工程治理、生产交付和 SRE 平台化的演进。
...
| 来源 | 用途 | 当前示例 |
| --- | --- | --- |
| Git 分支 | 正在开发哪条工作线 | `release/v2.5.0` |
| Maven `revision` 与前端 `package.json` | 当前构建产物声明的版本 | `v2.5.0` |
| Git tag | 已形成的正式发布锚点 | `v2.4.3`；`v2.5.0` 在发布 PR 合并后创建 |
...
因此不要仅凭分支名宣布发布，也不要因为 package 版本已经提升就提前宣布正式发布。当前统一口径是：**发布准备分支和构建产物为 `v2.5.0`；最近正式 tag 为 `v2.4.3`，发布 PR 合并后再创建 `v2.5.0` tag。**
...
发布 PR 合并后再创建 `v2.5.0` tag；详细变化以 [变更日志](/guide/changelog) 为准。
...
- **当前发布线**：`release/v2.5.0`
- **当前构建产物版本**：`v2.5.0`，以 Maven `revision` 和各前端 `package.json` 为准
- **最近正式 tag**：`v2.4.3`，发布 PR 合并后创建 `v2.5.0`

...
| 来源 | 用途 | 当前示例 |
| --- | --- | --- |
| Git 分支 | 正在开发哪条工作线 | `feature/sre-24x7-platform` |
| Maven `revision` 与前端 `package.json` | 当前构建产物声明的版本 | `v2.5.4` |
| Git tag | 已形成的正式发布锚点 | `v2.4.3` 及之前的历史 tag；`v2.5.4` 待发布 |
...
因此不要仅凭分支名宣布发布，也不要因为 package 版本尚未提升就忽略正在进行的开发线。当前统一口径是：**构建产物为 `v2.5.4`；工作树尚未形成新的正式发布 tag，完成发布前验证后再创建 `v2.5.4` tag。**
...
- **当前发布线**：`v2.5.4`
- **当前构建产物版本**：`v2.5.4`，以根目录 `VERSION`、Maven `revision` 和各前端 `package.json` 为准
- **最近正式 tag**：历史 `v2.4.3`；`v2.5.4` 完成验证后创建

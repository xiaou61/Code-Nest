...
| 想直接拿现成表单跑带教 | [带教交付模板包](/guide/onboarding-deliverable-templates) |
| 学完后准备接第一个真实任务 | [首个真实任务接入指南](/guide/first-real-task) |
| 想直接给新人分配第一单 | [首单任务题库](/guide/first-task-backlog) |
| 想判断什么时候可以继续接手一个模块 | [模块接手升级路线](/guide/module-ownership-ladder) |
| 想真正接手模块前先盘清楚入口、边界和验证 | [模块接手检查清单](/guide/module-takeover-checklist) |
| 想直接对照真实模块类型看接手记录应该怎么写 | [模块接手样例包](/guide/module-takeover-examples) |
| 想把日常值守、回归、发版前验证和临时交接跑起来 | [模块值守与回归手册](/guide/module-owner-playbook) |
...
| 想知道一个接口到底是公开、用户态、管理员态还是 owner 归属校验 | [权限注解与角色边界索引](/reference/permission-boundaries) |
| 想按失败表现反查应该先看哪层 | [异常路径与失败态索引](/reference/failure-paths) |
| 想查某个动作成功后应该回流到哪里 | [事件、通知与回流索引](/reference/event-backflow-index) |
| 想查某个数字为什么不对、排行怎么来的、口径怎么对账 | [统计、排行与计数口径索引](/reference/statistics-ranking-counts) |
| 想判断重复提交、失败回滚和后续补偿 | [幂等、回滚与补偿索引](/reference/idempotency-rollbacks-compensation) |
| 想先搞清楚某个复杂模块的状态到底怎么推进 | [模块状态机与生命周期索引](/reference/module-state-machines) |
| 选择测试和回归 | [测试与回归](/guide/testing-regression)、[发布前验证](/guide/release-verification) |
| 想快速查某个模块改完以后最低应该回归什么 | [模块最小回归矩阵](/reference/module-regression-matrix) |
| 线上告警、需要立即止损 | [事故响应](/operations/incident-response)、[监控与观测](/operations/monitoring) |
...
| 基础能力 | 被哪些模块调用 | 典型场景 | 影响面 |
| --- | --- | --- | --- |
| 鉴权 | 几乎所有用户端和管理端接口 | 登录、角色校验、当前用户 ID | 全局 |
| 用户账户 | 积分、通知、社区、聊天、学习统计 | 注册、资料、头像、状态 | 中 |
| 文件存储 | 头像、简历导出、聊天图片、内容附件 | 上传、访问 URL、迁移 | 中 |
| 通知中心 | 注册、资料变更、社区互动、审核结果 | 站内信、公告、模板消息 | 中 |
| 积分 | OJ、签到、博客、抽奖、CodePen | 奖励、扣减、流水 | 中 |
| 敏感词 | 社区、动态、评论、内容发布 | 拦截、替换、记录 | 中 |
| AI Runtime | 模拟面试、求职作战台、SQL 优化、社区摘要 | Prompt、Schema、RAG、回归 | 中 |
| 前端渲染安全 | 社区、博客、题库、OJ、闪卡、通知、开发者工具 | Markdown、`v-html`、高亮和 Diff 展示 | 中 |
...
| SRE 运维 | 告警、事故、证据、只读 RCA、反馈与调查审计 | `xiaou-sre`、`xiaou-system` |
...
| 成长闭环 | [题库与成长闭环](/modules/interview-and-growth)、[社区与内容矩阵](/modules/community-content) |
| 公共能力 | [公共模块](/modules/common) |

...
| 模块 | 关系 | 说明 |
| --- | --- | --- |
| [公共底座](/modules/common) | 强依赖 | 积分和抽奖依赖公共底座的 Redis、并发工具和异常处理 |
| [用户账户与个人中心](/modules/user-account) | 强依赖 | 积分余额关联用户账户，头像和昵称展示依赖用户信息 |
| [OJ 判题系统](/modules/oj) | 被依赖 | 首次 AC 题目会触发积分发放（PointsType.OJ_AC） |
| [敏感词风控](/modules/sensitive) | 间接依赖 | 抽奖风控链中可能集成敏感词检测 |
| [通知中心](/modules/notification) | 间接依赖 | 中奖通知可能通过通知中心推送 |
| [统计、排行与计数口径索引](/reference/statistics-ranking-counts) | 参考 | 积分排行和统计口径定义 |

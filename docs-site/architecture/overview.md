...
读完本页后，可以用这几件事确认你已经抓住架构主线：

1. 能说清楚 `xiaou-application` 为什么是启动聚合层，而不是具体业务模块。
2. 能区分用户端、管理端、后端业务模块、公共模块和 API 契约模块的边界。
3. 能解释 MySQL、Redis、WebSocket、AI sidecar 在系统里各自解决什么问题。
4. 能从一个页面入口追到 Controller、Service、Mapper 和数据表。
5. 能说明 `docs-site/` 为什么独立部署，而不是混在原有 `docs/` 资料库里。

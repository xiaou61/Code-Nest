...
1. 根 `pom.xml` 是否声明了新模块或依赖关系。
2. Controller 是否放在业务模块，而不是塞进 `xiaou-application`。
3. 跨模块调用是否优先通过 `*-api` 契约，而不是直接依赖实现类。
4. 新表或字段是否有版本 SQL，并同步到数据表索引。
5. 对应模块页是否补了源码地图、核心流程、数据表和验证清单。

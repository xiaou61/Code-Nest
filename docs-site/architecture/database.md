...
Code-Nest 使用单一 MySQL 数据库 `code_nest`。当前 `sql/MySql/code_nest.sql` 已合并为 **154 张表**的完整结构基线；`sql/v*` 下的版本脚本用于旧环境升级和演进追溯，不需要在全新数据库上重复执行。
...
| `sre_` | xiaou-sre, xiaou-system | 9 | 告警、事故、证据、事务 Outbox、RCA 调查、步骤、反馈与脱敏回放产物 |
...
| [数据表索引](/reference/database-tables) | 当前 154 张表清单 |
